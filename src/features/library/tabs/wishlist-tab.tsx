  ...
  if (isLoading) {
    return (
      <div className="space-y-4 lg:space-y-6">
        {[...Array(3)].map((_, i) => (
          <Card key={i} className="bg-white/90 dark:bg-[#1e2847]/80 backdrop-blur-sm border-border/50 dark:border-white/10 animate-pulse">
            <CardContent className="py-8">
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="h-6 w-6 bg-[#1a2240]/10 dark:bg-white/10 rounded" />
                </div>
              </div>
            </CardContent>
          </Card>
    ...
    <div className="space-y-4 lg:space-y-6 max-w-2xl">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          <Heart className="w-6 h-6 text-[#1a2240] dark:text-white" />
          <h2 className="text-2xl font-semibold text-[#1a2240] dark:text-white">
            My Wishlist
          </h2>
        </div>
        <p className="text-sm text-[#4e5a7e] dark:text-white/60">
          {wishlistItems.length} item{wishlistItems.length !== 1 ? "s" : ""} you're interested in purchasing
        </p>
      </div>
